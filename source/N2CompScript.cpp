...
void N2CompScript::LoadFromBin(const ur::Device& dev, const std::string& dir, bs::ImportStream& is)
{
	// todo
}
...
void N2CompScript::LoadFromJson(const ur::Device& dev, mm::LinearAllocator& alloc, const std::string& dir, const rapidjson::Value& val)
{
	auto filepath = val["filepath"].GetString();
	m_filepath = boost::filesystem::absolute(filepath, dir).string();
}

void N2CompScript::StoreToMem(const ur::Device& dev, n2::CompScript& comp) const
{
